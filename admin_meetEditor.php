...
enforceSessionPolicies($conn); // Enforce session timeout + role sync before allowing admin editing
...
// Meet to edit can arrive via GET (initial load) or via POST (form submissions)
$meetID = null;
...
// Parse flexible time input into centiseconds (supports formats like SS, SS.hh, M:SS, M:SS.hh)
function parseTimeToCentiseconds(string $raw): ?int {
    ...
    // Split out minutes if provided (e.g., "1:05.32")
    if (strpos($s, ':') !== false) {
    ...
    // Parse seconds and optional fractional (hundredths) part
    $seconds = 0;
    $hundredths = 0;
    if (strpos($secPart, '.') !== false) {
        ...
        // Keep the first two fractional digits; pad 1-digit fractions (".3" -> ".30")
    ...
    // Enforce seconds range to prevent invalid time strings
    if ($seconds < 0 || $seconds > 59) return null;

    return ($minutes * 60 + $seconds) * 100 + $hundredths;
}

// Convert centiseconds back to canonical "MM:SS.hh" string for DB storage/display consistency
...
// Fetch meet row used across the editor (details + course + flags)
...
// Fetch all events for a given course (includes INDIV vs RELAY so the UI can split editors)
...
// Fetch just event IDs currently attached to the meet (used to render checkbox state + compute add/remove)
...
// Fetch full event info for events attached to the meet (used for time editor dropdowns)
...
// Fetch swimmers for selection in the time editor (includes current year group for auto-fill)
...
// Fetch recorded individual results for this meet (includes yeargAtEvent so it can be viewed/edited)
...
/* Relay helpers (meetID+eventID is the identity for a relay team) */

// Fetch relay team summary rows for the meet (one team per relay event)
...
// Fetch relay members for one relay event team (ordered by leg for display)
...
// All editor actions post back to this same page and are distinguished by the hidden "action" field
if ($meetID && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';

    try {
        /* Update meet metadata */
        ...
        /* Attach/detach events for this meet */
        ...
        if ($action === 'save_events') {
            ...
            // Only allow events that match the meet course (prevents attaching wrong-course events)
            ...
            // Compute diff for insert/delete operations
        ...
        /* Individual times: add/update/delete */
        ...
        if ($action === 'add_time' || $action === 'update_time') {
            ...
            // Ensure the event is attached to this meet before allowing a result entry
            ...
            // Ensure this is an individual event (relay events are handled separately below)
            ...
            // Normalize time input to canonical storage format
            ...
            if ($action === 'add_time') {
                // Determine year group at event time (optional input; if blank, snapshot swimmer current year group)
                ...
                // Insert time row (or update time if it already exists) and store yeargAtEvent on insert
                ...
                // update_time: always updates time; optionally updates yeargAtEvent if the form indicates a change
                $updateYear = false;
                $newYear = null;
                ...
                // Update the time row (and yeargAtEvent only when requested/changed)
        ...
        if ($action === 'delete_time') {
            ...
            // Delete a single swimmer+event time from this meet
        ...
        if ($action === 'create_relay') {
            ...
            // Ensure the event is attached to the meet and is a relay event
            ...
            // Normalize relay total time to canonical format
            ...
            // Insert or update relay team for this meet+event
        ...
        if ($action === 'add_member') {
            ...
            // Insert or update a relay member for a given leg (leg identity is meetID+eventID+leg in schema)
        ...
        if ($action === 'delete_relay') {
            ...
            // Delete the relay team record for this meet+event (members may be left in tblrelayTeamMember depending on schema constraints)
...
// Relay teams for this meet (one per relay event)
$relayTeams = $meetID ? getRelayTeams($conn, $meetID) : [];

// Build swimmer options used by the client-side datalist search (includes gender + yearg for eligibility and auto-fill)
...
<div class="main-content">
    ...
    <div class="section">
        ...
        <!-- Meet details edit form (only shown when a valid meet is loaded) -->
        <?php if ($meet): ?>
    ...
    <?php if ($meet): ?>
    <!-- Event attachments section: determines which events can receive results for this meet -->
    <div class="section">
    ...
    <!-- Individual time entry and editing section -->
    <div class="section">
        <h2>Times</h2>
        ...
        <div class="sub-section">
            ...
            <form method="post" class="inline-form wrap" id="addTimeForm">
                ...
                <!-- Event dropdown is restricted to INDIV events attached to this meet -->
                ...
                <!-- Swimmer search uses a datalist; actual userID is stored in a hidden input for POST -->
                ...
                <!-- Optional year-at-event override; if blank, UI auto-fills from swimmer yearg -->
                ...
                <!-- Time input is normalized server-side into canonical MM:SS.hh -->
                ...
                <?php else: ?>
                    <?php foreach ($times as $row): ?>
                        <tr>
                            ...
                            <td>
                                <!-- Inline form allows updating time and yeargAtEvent for a single result row -->
    ...
    <div class="section">
        ...
        <div class="sub-section">
            ...
            <form method="post" class="inline-form wrap">
                ...
                <!-- Relay event dropdown is restricted to RELAY events attached to this meet -->
                ...
                <?php else: ?>
                    <?php foreach ($relayTeams as $rt): ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                // Per-team member list query (meetID + eventID identify the team)
...
<script>
// Client-side swimmer search/selection helpers for the "Add a Time" form
...
// Rebuild swimmer datalist based on the selected event gender (enforces eligibility in UI)
function rebuildSwimmerDatalist(requiredGender) {
    ...
    // - Male events allow swimmers with gender 'M' or 'MIX'
    // - Female events allow swimmers with gender 'F'
    // - If no gender is provided, allow all swimmers
    SWIMMERS.forEach(sw => {
        const isEligible =
            !requiredGender ||
    ...
    // Reset selected swimmer fields when event changes
    swimmerSearch.value = '';
    userIDHidden.value = '';
    if (yeargAtEventInput) yeargAtEventInput.value = '';
}

// When event changes, rebuild swimmer suggestions using event gender filter
...
// Resolve swimmerSearch label into a concrete userID and auto-fill yeargAtEvent if empty
function handleSwimmerChosen() {
    ...
    // Fallback lookup in case the datalist option isn't directly matched
    if (!chosenId) {
    ...
    // Auto-fill year-at-event from swimmer yearg when the year input is blank
    if (yeargAtEventInput && (!yeargAtEventInput.value || yeargAtEventInput.value.trim() === '') && chosenYear) {
        yeargAtEventInput.value = chosenYear;
    }
}

// Validate the add-time form before submitting (requires swimmer chosen from suggestions and valid year range)
...
// Confirm before updating a stored year-at-event value in the existing-times table
...
// Wire up event listeners and initialize datalist based on the currently selected event (if any)
if (eventSelect) {
